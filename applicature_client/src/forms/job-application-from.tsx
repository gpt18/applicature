...
function JobApplicationForm({ className }: React.ComponentProps<'form'>) {
    ...
    const [loadingCompanyCardPreview, setLoadingCompanyCardPreview] = useState(false);
    ...
    const handleSubmit = (event: React.FormEvent) => {
        ...
        if (validateForm()) {
            let updatedApplications = JSON.parse(localStorage.getItem('myApplications') || '[]');
    ...
    return (
        <form className={cn("grid items-start gap-4", className)}>
            <div className="grid gap-2 md:px-2">
                ...
                <Select onValueChange={(value) => handleStatusChange(value)}>
                    <SelectTrigger className="w-[180px]">
                        <SelectValue placeholder={applicationStatus?.find(s => s?.value === newJob?.timeline?.[0]?.status)?.label || "Select Application Status"} />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <ScrollArea className='max-w-[200px] py-3'>
                    <div className='flex h-fit gap-2'>
                        {jobTypes.map((jobType) => (
                            <Badge
                                key={jobType.value}
                                variant={newJob?.jobType === jobType.value ? 'default' : 'outline'}
                    ...
                    <Popover>
                        <PopoverTrigger asChild>
                            ...
                            >
                                ...
                                {newJob?.appliedOn ? format(newJob?.appliedOn, "PPP") : <span>Pick a date</span>}
                            </Button>
                        </PopoverTrigger>
                        <PopoverContent className="w-auto p-0">
                            <Calendar
                                ...
                                onSelect={(date) => setNewJob({ ...newJob, appliedOn: date?.toISOString() || '' })}
                                initialFocus
                            />
                        </PopoverContent>
                    </Popover>
            ...
            <div className="grid gap-2 md:px-2">
                ...
                />
                {newJob?.jobLink !== '' && <Button type="button" onClick={() => getCompanyDetails({ companyUrl: newJob?.jobLink })}>Get Company Details</Button>}
            </div>
            {(
                <div className="p-4 flex gap-2.5 items-center border rounded-md">
                    <div className="">
                        {loadingCompanyCardPreview ? <LoaderCircle className={cn('animate-spin')} /> :
                            <img src={newJob?.logo} onError={
                                (e: any) => {
                    ...
                    <div className="border-l pl-2.5 flex flex-col justify-center">
                        <h1 className="text-lg font-bold line-clamp-1" title={newJob?.companyName}>{newJob?.companyName || "Company name"}</h1>
                        <p className="text-xs">{isValidUrl(newJob?.domain) ? new URL(newJob?.domain).hostname : newJob?.domain || "domain"}</p>
                    </div>
                </div>
            )}
            <div>
                <Accordion type="single" collapsible>
                    <AccordionItem value="item-1" className='border rounded-md p-2 bg-slate-50'>
                        ...
                        <AccordionContent className='grid items-start gap-4'>
                            <div className="grid gap-2 md:px-2">
                                ...
                                <Input
                                    ...
                                    onChange={handleInputChange}
                                />
                                {newJob?.domain !== '' && <Button type="button" onClick={() => getCompanyIconFromDomain({ domain: newJob?.domain })}>Update Logo</Button>}
                            </div>
                            <div className="grid gap-2 md:px-2">
            ...
            <div className="grid gap-2 md:px-2">
                ...
                <Select onValueChange={(value) => setNewJob({ ...newJob, workType: value })}>
                    <SelectTrigger className="w-[180px]">
                        <SelectValue placeholder={jobWorkTypes.find(i => i.value === newJob?.workType)?.label || "Select Work Type"} />
                    </SelectTrigger>
                    <SelectContent>
            ...
            <Button type="button" onClick={handleSubmit}>
                {applicationStatus?.find(as => as.value === newJob?.timeline?.[0]?.status)?.label || "Save"}
            </Button>
        </form>
    );
}
