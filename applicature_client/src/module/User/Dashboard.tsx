...
import JobApplicationForm, {
  ...
  applicationStatus,
  jobTypes,
} from "@/forms/job-application-from";
...
function Dashboard() {
  ...
  useEffect(() => {
    const myApplications = JSON.parse(
      ...
      <div>
        <div className="flex px-5 py-2 md:px-10 md:py-4 border-b items-center justify-between">
          ...
          <div className="flex items-center gap-1 md:gap-3">
            ...
            <img
              alt=""
              className="size-10 object-fit rounded-full bg-slate-400"
            />
          </div>
        </div>
        <div className="">
          ...
          {Applications?.length === 0 && (
            <div className="flex justify-center items-center flex-col text-center px-8 pt-52">
              ...
              <span className="text-lg">
                Click on the <b>Create New</b> button to add a new job
                application.
              </span>
            </div>
          )}
              ...
              <div className="flex justify-between py-3">
                <div className="flex place-items-center gap-2">
                  <span className="text-[10px] text-muted-foreground pr-1 border-r">
                    {
                      jobTypes.find((jt) => jt.value === item?.jobType)
                        ?.shortLabel
                    }
                  </span>
                ...
                <div className="text-muted-foreground text-sm flex items-center gap-2 divide-x">
                  ...
                  {item?.jobLink && (
                    <div
                      className="pl-2"
                      onClick={() => window.open(item?.jobLink, "_blank")}
                    >
                ...
                <div className="flex flex-col justify-center">
                  <div className="flex gap-1 items-center">
                    ...
                    <p className="font-bold">
                      {jobWorkTypes.find((jwt) => item?.workType === jwt?.value)
                        ?.label || "-"}
                    </p>
                  </div>
                  <div className="flex gap-1 items-center">
            ...
            <div className="border-t flex justify-between px-4 py-2">
              <div className="text-xs md:text-sm text-gray-500">
                <span className="flex items-center">
                  ...
                  />
                  {new Date(item?.appliedOn).toLocaleDateString("en-US", {
                    weekday: "short",
                    day: "2-digit",
                    month: "short",
                    year: "numeric",
              ...
              {viewBtn && (
                <DetailedView
                  trigger={
                    ...
                    >
                      <Eye size={15} className="mr-1 md:mr-2 text-gray-500" />
                      View
                    </Badge>
                  }
                  item={item}
    ...
    <Card className="overflow-hidden">
      <CardContent className="p-0 flex flex-col justify-center rounded-lg">
            ...
            <div className="py-2">
              {item?.timeline?.map((tml: any, i: number) => (
                <div key={i} className="flex flex-col items-center">
                  <Badge
                    variant={"outline"}
                    className="w-fit bg-yellow-100 text-xs"
                  >
                    {tml?.status}
                  </Badge>
                  <span className="text-xs text-muted-foreground text-nowrap">
                    {new Date(tml?.timestamp).toLocaleString()}
                  </span>
              ...
              <div className="flex flex-col items-center cursor-pointer">
                <Badge
                  variant={"outline"}
                  className="w-fit bg-white text-xs border-dashed items-center flex gap-1 justify-center hover:bg-gray-100 py-2"
                >
        ...
        <div className="p-4">
          ...
          <div className="py-2">
            <div
              className="rounded-lg border p-2 text-sm md:text-base break-words max-h-[400px] overflow-y-scroll"
              style={{ scrollbarWidth: "none" }}
            >
              <div className="text-gray-700">
                <p>
                  On {new Date(item?.appliedOn).toLocaleDateString()}, you
                  applied for a
                  <span className="font-semibold"> {item?.jobRole}</span>{" "}
                  position at
                  <span className="font-semibold"> {item?.companyName}</span>, a
                  <span className="font-semibold"> {item?.jobType}</span>{" "}
                  company located in
                  <span className="font-semibold"> {item?.location}</span>. The
                  role offers a competitive CTC of
                  <span className="font-semibold">
                    {" "}
                    ₹{(item?.ctc / 100000).toFixed(2)} LPA
                  </span>
                  .
                </p>
                <p>
      ...
      <div className="relative border rounded-lg flex">
        {asLink ? (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="p-2 flex-1 text-sm overflow-scroll text-nowrap text-blue-500 hover:underline cursor-pointer"
            style={{ scrollbarWidth: "none" }}
          >
            {text}
          </a>
          ...
          <div
            className={cn("p-2 flex-1 text-sm overflow-scroll text-nowrap")}
            style={{ scrollbarWidth: "none" }}
          >
            {text}
          </div>
    ...
    <Sheet>
      ...
      <SheetContent>
        <SheetHeader>
          ...
          <SheetDescription>
            Upload your resume here. Click save when you're done.
          </SheetDescription>
        </SheetHeader>
        <MyResources />
      </SheetContent>
    ...
    <div className="py-4">
      ...
      <div className="py-4">
        <div className="border rounded-md flex p-2 gap-2">
          ...
          <span className="text-sm flex-1 hover:underline cursor-pointer">
            Resume.pdf
          </span>
          <Clipboard
            size={20}
            className="m-auto text-muted-foreground cursor-pointer"
          />
        </div>
      </div>
    </div>
